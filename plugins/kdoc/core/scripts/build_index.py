...
GENERATED_LINE_RE = re.compile(
    r"^> Generated by `kdoc:build:index` on .+\. Do not edit manually\.$",
    re.MULTILINE,
)
...
def normalize_generated_index(text: str) -> str:
    return GENERATED_LINE_RE.sub(
        "> Generated by `kdoc:build:index` on <generated>. Do not edit manually.",
        text,
    ).strip()


def main() -> None:
    parser = argparse.ArgumentParser(description="Build or validate Knowledge/INDEX.md")
    parser.add_argument(
        "--check",
        action="store_true",
        help="Compare the generated index against the current file without rewriting it",
    )
    ...
    if previous is not None and normalize_generated_index(previous) == normalize_generated_index(content):
        if args.check:
    ...
    if args.check:
        if previous is None:
            print(f"OUTDATED: {output_file} is missing")
        else:
            print(f"OUTDATED: {output_file} does not match generated content")
        sys.exit(1)
